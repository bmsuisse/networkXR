...
```bash
uv run pytest tests/ -q -x --ignore=tests/test_benchmarks.py
```

4. Create the new tag (ensure you have bumped the version in `pyproject.toml` and `Cargo.toml` and committed the changes first):
```bash
# Replace vX.Y.Z with your new version
export NEW_VERSION="v0.1.1"
git tag $NEW_VERSION
```
